...
def kernel(matrix_x, sample_x, kernelOption):  
    ...
    if kernelType == 'linear':  
        ...
        R = kernelOption[1]
        d = kernelOption[2]
        kernelValue = power(matrix_x * sample_x.T + R,d)
    elif kernelType == 'rbf':  
        ...
        k = kernelOption[1]
        delta = kernelOption[2]        
        kernelValue = tanh(k * matrix_x * sample_x.T - delta)
    else:  
...
def calcKernelMatrix(train_x, kernelOption):  
    ...
    for i in xrange(numSamples):  
        kernelMatrix[:, i] = kernel(train_x, train_x[i, :], kernelOption)  
    return kernelMatrix  
...
def SMO(svm, alpha_i):  
    ...
    if (svm.train_y[alpha_i] * error_i < -svm.toler) and (svm.alphas[alpha_i] < svm.C) or (svm.train_y[alpha_i] * error_i > svm.toler) and (svm.alphas[alpha_i] > 0):  
        ...
        if abs(alpha_j_old - svm.alphas[alpha_j]) < svm.toler:  
...
def testSVM(svm, test_x, test_y):  
    ...
    for i in xrange(numTestSamples):  
        kernelValue = kernel(supportVectors, test_x[i, :], svm.kernelOpt)  
...
def showSVM(svm):  
    ...
    supportVectorsIndex = nonzero(svm.alphas.A > 0)[0]  
    #filterSupportVectors = supportVectorsIndex[supportVectorsIndex < svm.C]
    for i in supportVectorsIndex:  
        plt.plot(svm.train_x[i, 0], svm.train_x[i, 1], 'oy')  
        print i,svm.alphas[i,0]
...
if __name__ == '__main__':
    ...
    toler = 0.00001
    maxIter = 100
    classifier = trainSVM(train_x, train_y, C, toler, maxIter, kernelOption = ('linear',2.0,1))
    acc = testSVM(classifier, test_x, test_y)
    print acc
    showSVM(classifier)